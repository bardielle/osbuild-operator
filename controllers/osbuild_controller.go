...
func (r *OSBuildReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if lastBuildStatus == startedContainerBuild {
		...
		if composeStatus == composer.ComposeStatusValueFailure {
			err = fmt.Errorf("failed to build edge container")
			logger.Error(err, "")
			return ctrl.Result{}, err
		}

		// if the build was finished successfully and the target image is edge-container than return
		if composeStatus == composer.ComposeStatusValueSuccess && osBuild.Spec.Details.TargetImage.TargetImageType == edgeContainerImgType {
	...
	if lastBuildStatus == startedIsoBuild {
		...
		// the build was failed - return with error
		if composeStatus == composer.ComposeStatusValueFailure {
			err = fmt.Errorf("failed building the edge installer")
			logger.Error(err, "")
			return ctrl.Result{}, err
		}
...
func (r *OSBuildReconciler) postComposeEdgeContainer(ctx context.Context, logger logr.Logger, osBuild *osbuildv1alpha1.OSBuild) error {
	...
	if composerResponse.StatusCode() != http.StatusCreated {
		logger.Info(fmt.Sprintf("********************* BODY    %s", string(composerResponse.Body)))
		errorMsg := fmt.Sprintf("postCompose request was failed for OSBuild %s, with status code %v", osBuild.Name, composerResponse.StatusCode())
		err = fmt.Errorf(errorMsg)
		logger.Error(err, "postCompose request was failed")
...
func (r *OSBuildReconciler) updateOSBuildStatus(ctx context.Context, logger logr.Logger, osBuild *osbuildv1alpha1.OSBuild,
	msg string, typeCondition osbuildv1alpha1.OSBuildConditionType, containerComposeId string, isoComposeId string) error {
	...
	if conditionArrLen > 0 {
		...
		if lastConditionType == typeCondition {
			logger.Info("The conditionType was left the same as before ", " lastConditionType ", lastConditionType)
			return nil
		}
	}
...
func (r *OSBuildReconciler) createImageRequest(targetImage *osbuildv1alpha1.TargetImage, targetImageType osbuildv1alpha1.TargetImageType) *composer.ImageRequest {
	...
	// TODO - add repositories to osbuildConfig and osbuildConfigTemplate types
...
func (r *OSBuildReconciler) createCustomizations(osbuildCustomizations *osbuildv1alpha1.Customizations) *composer.Customizations {
	if osbuildCustomizations == nil {
		return &composer.Customizations{}
	}
	...
	if osbuildCustomizations.Services != nil {
		...
		if osbuildCustomizations.Services.Enabled != nil {
			services.Enabled = &osbuildCustomizations.Services.Enabled
		}
		if osbuildCustomizations.Services.Disabled != nil {
